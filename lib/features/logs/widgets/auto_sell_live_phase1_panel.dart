...
class AutoSellLivePhase1Panel extends StatelessWidget {
  const AutoSellLivePhase1Panel({super.key, required this.controller});
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final result = controller.autoSellLivePhase1Status ??
            controller.autoSellLivePhase1Result;
        final loading = controller.autoSellLivePhase1Loading;
        return Container(
          key: const ValueKey('auto-sell-live-phase1-panel'),
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          result == null
                              ? '${strings.limitedLiveAutoSell} / ${strings.disabledByDefault}'
                              : '${strings.statusLabel(result.resultStatus)} / ${result.selectedSymbol ?? result.primaryBlockReason ?? strings.limitedLiveAutoSell}',
                  ...
                  IconButton(
                    key: const ValueKey('refresh-auto-sell-phase1-status'),
                    tooltip: strings.refreshAutoSellPhase1Status,
                    onPressed: loading
                        ? null
              ...
              _BadgeWrap(badges: strings.autoSellPhase1Badges),
              if (controller.autoSellLivePhase1Error != null) ...[
                const SizedBox(height: 10),
                Text(
                  controller.autoSellLivePhase1Error!,
    ...
    return Column(
      ...
      children: [
        _StatusBanner(
          ...
          detail: strings.noAutoSellYet,
          color: Colors.white54,
        ),
        const SizedBox(height: 10),
        _DetailRow(
          label: strings.exitCandidateRequired,
          value: strings.stopLossTakeProfitCandidate,
        ),
        _DetailRow(
...
class _Phase1ResultBlock extends StatelessWidget {
  ...
  final AutoSellLivePhase1Result result;
  final AppStrings strings;
      ...
      children: [
        _StatusBanner(
          ...
          label: result.submitted
              ? strings.autoSellSubmitted
              : result.blocked
                  ? strings.autoSellBlocked
                  : strings.autoSellResult,
        ...
        Wrap(
          ...
          children: [
            ...
            _Metric(
              label: strings.selectedExitCandidate,
              value: result.selectedCandidateId ?? '-',
            ),
            _Metric(
              label: strings.dailyLimit,
              value:
                  '${result.dailyAutoSellCount}/${result.dailyAutoSellLimit}',
            ),
            _Metric(
        ...
        const SizedBox(height: 10),
        _DetailRow(
          label: strings.heldPositionsOnly,
          value: strings.riskReductionOnly,
        ...
        _DetailRow(
          label: strings.candidateType,
          value: result.candidateType ?? '-',
        ),
        _DetailRow(
          label: strings.candidateSeverity,
          value: result.candidateSeverity == null
              ? '-'
              : strings.statusLabel(result.candidateSeverity!),
        ),
        _DetailRow(
        ...
        _DetailRow(
          label: strings.sellPreflightStatus,
          value: result.sellPreflightStatus == null
              ? '-'
              : strings.statusLabel(result.sellPreflightStatus!),
        ),
        _DetailRow(
          label: strings.usedRemaining,
          value: '${result.dailyAutoSellCount} / ${result.dailyRemaining}',
        ),
...
class _ActionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: [
        OutlinedButton.icon(
          key: const ValueKey('refresh-auto-sell-phase1-status-action'),
          onPressed: loading ? null : onRefresh,
          icon: const Icon(Icons.refresh, size: 18),
          label: Text(strings.refreshAutoSellPhase1Status),
        ),
        OutlinedButton.icon(
          key: const ValueKey('run-auto-sell-phase1-once'),
          onPressed: loading ? null : onRunOnce,
          icon: const Icon(Icons.play_arrow_outlined, size: 18),
          label: Text(strings.runPhase1SellAttemptOnce),
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (latestRun.selectedCandidateId != null)
          _DetailRow(
            label: strings.selectedExitCandidate,
            value: latestRun.selectedCandidateId!,
          ),
...
class _ChecklistBlock extends StatelessWidget {
  ...
  final List<AutoSellLivePhase1ChecklistItem> items;
  final AppStrings strings;
...
Color _statusColor(AutoSellLivePhase1Result result) {
