...
def test_kis_gpt_prompt_includes_event_context_and_not_bullish_rule():
    ...
    assert "USD/KRW FX risk" in call["instructions"]
    assert "Previous US market session" in call["instructions"]
    assert "SOX semiconductor sentiment" in call["instructions"]
    assert "Foreign and institutional investor flow" in call["instructions"]
    assert "Positive news cannot create a buy signal alone" in call["instructions"]
