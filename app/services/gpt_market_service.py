...
class GPTMarketService:
    ...
    def _load_context_from_cache(self, db: Session, symbol: str) -> MarketGateContext:
        ...
        if used_cache:
            return MarketGateContext(cached_site_summaries=summaries, used_cache=True)

        refreshed = self._best_effort_refresh_cache(db, symbol)
        if refreshed > 0:
            summaries, used_cache = self.cache_service.get_fresh_summaries(db, symbol)
    ...
    def _best_effort_refresh_cache(self, db: Session, symbol: str) -> int:
        try:
            ...
            if not summaries:
                return 0
            return self.cache_service.upsert_summaries(db, symbol, summaries)
        except Exception:
            return 0
